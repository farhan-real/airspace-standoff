...
class AfterActionReplay {
  ...
  static encode(frames, targetBytes = 90 * 1024) {
    ...
    const packEntities = (list, group, pack) => (list || []).map(entity => pack(entity, addDefinition(group, entity, pack.definition)));
    const packAircraft = (entity, index) => [
      index, q(entity.x), q(entity.y), q(entity.heading, 1000), q(entity.speed, 100),
    ...
    const groupConfig = {
      surfaces: { key: 'surfaceUnits', pack: (e, i) => [i, q(e.x), q(e.y), q(e.hp)], definition: e => [e.team || 'hostile', e.type || '', e.name || '', q(e.maxHp), e.isIndestructible ? 1 : 0] },
      missiles: { key: 'missiles', pack: (e, i) => [i, q(e.x), q(e.y), q(e.heading, 1000), q(e.speed, 100)], definition: e => [e.team || 'hostile', e.weapon || 'MISSILE'] },
      civilians: { key: 'civilianTraffic', pack: (e, i) => [i, q(e.x), q(e.y), q(e.heading, 1000), q(e.speed, 100), q(e.hp), e.identified ? 1 : 0], definition: e => [e.flightCode || 'CIVILIAN'] },
      decoys: { key: 'decoys', pack: (e, i) => [i, q(e.x), q(e.y), q(e.heading, 1000), q(e.speed, 100), q(e.hp)], definition: e => [e.team || 'friendly', e.model || 'DECOY'] }
    };
    ...
    for (const stride of strides) {
      ...
      const packedFrames = sampleIndexes.map(index => {
        ...
        Object.entries(groupConfig).forEach(([group, config]) => {
          row.push(packEntities(frame[config.key], group, Object.assign((entity, entityIndex) => config.pack(entity, entityIndex), { definition: config.definition })));
        });
  ...
  static decode(replay) {
    ...
    const unpack = (rows, definitions, build) => (rows || []).map(row => {
      const definition = definitions[row[0]] || [];
      return build(definition, row);
    });
    return replay.frames.map(row => {
      const aircraft = (rows) => unpack(rows, defs.aircraft || [], (d, r) => ({
        id: r[0], callsign: d[0], model: d[1], category: d[2], maxHp: d[3] / 10,
      ...
      const surfaces = unpack(row[3], defs.surfaces || [], (d, r) => ({
        id: r[0], team: d[0], type: d[1], name: d[2], maxHp: d[3] / 10,
        isIndestructible: Boolean(d[4]), x: r[1] / 10, y: r[2] / 10, hp: r[3] / 10
      }));
      const missiles = unpack(row[4], defs.missiles || [], (d, r) => ({
        id: r[0], team: d[0], weapon: d[1], x: r[1] / 10, y: r[2] / 10,
        heading: r[3] / 1000, speed: r[4] / 100
      }));
      const civilians = unpack(row[5], defs.civilians || [], (d, r) => ({
        id: r[0], flightCode: d[0], x: r[1] / 10, y: r[2] / 10,
        heading: r[3] / 1000, speed: r[4] / 100, hp: r[5] / 10, identified: Boolean(r[6])
      }));
      const decoys = unpack(row[6], defs.decoys || [], (d, r) => ({
        id: r[0], team: d[0], model: d[1], x: r[1] / 10, y: r[2] / 10,
        heading: r[3] / 1000, speed: r[4] / 100, hp: r[5] / 10
      }));
      return {
        time: row[0] / 10, alliedAircraft: aircraft(row[1]), hostileAircraft: aircraft(row[2]),
  ...
  updateButtons() {
    ...
    if (this.speedButton) this.speedButton.textContent = `${this.speed}× SPEED`;
