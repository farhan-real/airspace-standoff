 ...
 * Uniquely randomizes hostile airframe rosters, Aces, loadouts, and formations on every sortie.
 */
...
const FleetGenerator = {
  generateHostileFleet(difficultyKey, doctrineKey, theaterWidth, theaterHeight) {
    ...
    const maxPlanes = Math.max(3, basePlanes + Math.floor(Math.random() * 3 - 1));
    ...
    const shuffledAces = [...aceCandidates].sort(() => Math.random() - 0.5);
    const shuffledAceCallsigns = [...aceCallsigns].sort(() => Math.random() - 0.5);

    const hostileSquadron = [];
    let spentBudget = 0.0;
    const callsigns = [...(window.CALLSIGN_POOL || ['Viper', 'Ghost', 'Talon', 'Reaper', 'Bandit'])].sort(() => Math.random() - 0.5);
    ...
    for (let a = 0; a < aceQuota; a++) {
      ...
      if (catalog[aceSpecId] && (spentBudget + catalog[aceSpecId].cost <= targetBudget)) {
        const spawnX = (theaterWidth - 10.0) - Math.random() * 22.0;
        const spawnY = 14.0 + Math.random() * (theaterHeight - 28.0);
        const heading = Math.PI + (Math.random() * 0.35 - 0.175);
        const aceCallsign = shuffledAceCallsigns[a % shuffledAceCallsigns.length] || `★ Ace ${a + 1} ★`;
        const aceAltFt = (aceSpecId === 'DARKSTAR') ? 58000 : (32000 + Math.floor(Math.random() * 12) * 1000);
        ...
        const chosenAceWeapons = aceLoadoutProfiles[Math.floor(Math.random() * aceLoadoutProfiles.length)];
    ...
    const formationStyle = ['SPREAD_WALL', 'DOUBLE_COLUMN', 'PINCER_BRACKET', 'ECHELON'][Math.floor(Math.random() * 4)];
    ...
    for (let i = acesSpawned; i < maxPlanes; i++) {
      ...
      let spawnX = (theaterWidth - 12.0) - Math.random() * 18.0;
      let spawnY = midY;

      if (formationStyle === 'SPREAD_WALL') {
        const span = theaterHeight - 24.0;
        spawnY = 12.0 + ((i - acesSpawned) / Math.max(1, maxPlanes - acesSpawned - 1)) * span + (Math.random() * 4.0 - 2.0);
      } else if (formationStyle === 'PINCER_BRACKET') {
        const isUpper = (i % 2 === 0);
        spawnY = isUpper ? (14.0 + Math.random() * 22.0) : (theaterHeight - 14.0 - Math.random() * 22.0);
        spawnX -= (Math.random() * 10.0);
      } else if (formationStyle === 'ECHELON') {
        const step = (i - acesSpawned);
        spawnX -= step * 3.0;
        spawnY = 16.0 + step * (theaterHeight / (maxPlanes + 1)) + (Math.random() * 3.0 - 1.5);
      } else {
        spawnY = 14.0 + Math.random() * (theaterHeight - 28.0);
      }
      ...
      const randomizedHeading = Math.PI + (Math.random() * 0.30 - 0.15);
      const hostileAltFt = (chosenId === 'DARKSTAR') ? 58000 : (20000 + Math.floor(Math.random() * 16) * 1000);
  ...
  generateDynamicSquadronWave(waveIndex, team, theaterWidth, theaterHeight, difficultyKey) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const spawnX = isBlue ? (8.0 + Math.random() * 18.0) : (theaterWidth - 8.0 - Math.random() * 18.0);
      const spawnY = 14.0 + Math.random() * (theaterHeight - 28.0);
      const baseHeading = isBlue ? (Math.random() * 0.3 - 0.15) : (Math.PI + (Math.random() * 0.3 - 0.15));
      const waveAltFt = (specId === 'DARKSTAR') ? 58000 : (24000 + Math.floor(Math.random() * 12) * 1000);
      ...
      const wpn1 = isLead ? (Math.random() < 0.5 ? 'AIM-260' : 'METEOR') : 'AIM-120D';
      const wpn2 = Math.random() < 0.5 ? 'AIM-9X-2' : 'PYTHON-5';
    ...
    squadron.forEach(ac => {
      ...
      while (ac.getUsedSlots() < ac.totalSlots && attempts < 12) {
        ...
        const pool = (doctrine === 'STANDOFF')
          ? bvrMissiles
          : (Math.random() < 0.65 ? dogfightMissiles.concat(bvrMissiles) : bvrMissiles);
      ...
      if (allowedUpgrades > 0 && ac.equippedUpgrades.length < allowedUpgrades) {
        const upgPool = Object.keys(upgradesCatalog).sort(() => Math.random() - 0.5);
